...
查询接口为 `QueryBillOverview`，统计范围为 AccessKey 所属账户。默认用于费用展示；显式设置美元阈值后，也可作为实例调度的额外保护条件。账户创建及基础权限见[控制台准备与最小权限](permissions.md)。
...
| `billing.threshold_usd` | 正数或 `null` | `null` | 每个账号共用的本月 USD 保护阈值；`null` 不参与调度，正数启用保护且要求 `enabled: true`。 |
| `billing.refresh_interval_min` | 正整数 | `60` | 仅启用阈值后，后台定期刷新账单的间隔（分钟），启动后也刷新一次。 |
| `billing.stale_sec` | 正整数 | `10800` | 保护快照有效期（秒），不得小于刷新间隔；过期显示未知，已有超额证据仍生效。 |
...
普通登录用户需为上述命令添加 `sudo`。仅修改 `config.yaml` 不需要执行 `systemctl daemon-reload`。启动时会根据开关注册命令菜单；账单查询在收到命令或执行每日任务时触发。启用金额阈值后，额外注册启动后立即执行及周期性的后台账单刷新任务。
...
启用阈值后的周期查询也复用同一个账单查询锁。巡检只读取 SQLite 中的预算快照，不等待账单网络请求；周期查询本身不发送每小时报告。每个账号每月的账单超额只通知一次，通知标记写入 SQLite，刷新快照、重启服务或调整阈值都不会在同月重复通知；新月再次超额可以重新通知。金额未知的查询异常仍按原告警间隔通知。每日总结和 `/check` 持续显示预算状态，实际切换、停机和操作失败通知保留。
...
| 阈值保护周期任务 | 主动刷新；与已在执行的查询重叠时复用等待期间完成的结果。 |
| 每日查询等待已执行中的查询完成 | 复用等待期间完成的账户结果，避免重复请求。 |
| 跨月后的下一次查询 | 清除上月缓存，按新月份查询。 |
| 进程重启 | 内存缓存清空，下一次查询重新请求接口。 |

`/bill` 与 `/bill 账号键` 共用账户格式化缓存，不保留原始响应。这部分缓存仅存内存；启用阈值后，另将每账号每月的预算保护金额、最高已知金额、查询时间和错误状态写入 SQLite `bill_budget` 表，不保存完整账单明细。
...
失败不计为 0 元，旧数据不作为本次刷新成功的结果。权限不足、网络超时和响应解析错误本身不触发停机、切换或熔断；如果同月已确认超额，该证据仍参与保护，不因本轮失败解除。原始 SDK 异常及响应不直接输出，避免泄露请求中的敏感信息。
...
例如每个账号达到 10 美元时触发保护（10 仅为配置示例，请按自己的预算调整）：
...
保护金额为该账户本月 USD 预付费、按量付费和正向调账的税前金额之和，包含其他产品费用，并非受管 ECS 单台成本。退款与负向金额不抵扣这个保护金额；原始账单报告仍按币种和账单类型保留原符号列示。该口径是保守的停机保护指标，不是净应付、现金实付或结算总额。

- 当保护金额 **大于或等于**阈值时，该账号不再进入候选名单；当班账号超额时，按原有策略选候选，先启动并确认新机，再停止旧机。没有候选时执行全局保护停机。切换中发现目标超额也会重新选机。
- `/switch 账号键` 和 `/resume` 不能绕过已知超额限制。优先级仅在通过所有资格筛选的候选中比较。
- 同月最高已知保护金额写入数据库，查询失败、金额回落、过期或重启都不会自动解除超额限制。可调高阈值并重启，或等待新月份重新判断；关闭阈值也会停止使用此保护条件。全局保护停机后，调整阈值仍需 `/resume`，不会因配置放宽立即自动开机。
- 新月不使用上月金额和超额证据；历史记录按 `runtime_retention_months` 清理。月初恢复仍遵循原有机制，不会因为账单新月开始而主动从正常当班的 C 回切 A/B。
- 空账单、仅退款条目、非 USD 币种、未知账单类型、查询失败或快照过期会显示“金额未知”并告警。没有已有超额证据时，沿用流量和时长规则，不仅凭账单未知排除候选或停机；有既往同月超额证据时继续保护。
- `/check`、`/bill` 和每日报告显示保护金额、阈值或未知状态。预算查询不占用状态机控制锁；每分钟巡检只读取本地数据。

默认每小时刷新是本项目的轮询选择，不是阿里云保证的数据更新周期。账单入账存在延迟，频繁请求不能消除上游延迟；[官方实例账单说明](https://www.alibabacloud.com/help/en/user-center/developer-reference/api-bssopenapi-2017-12-14-describeinstancebill)还明确指出当月数据不包含未出账的按量费用。该说明针对实例账单接口，不能当作 `QueryBillOverview` 的精确更新承诺。因此账单保护应与流量、运行时长阈值配合，不能保证实际费用不超过配置金额。停机也不会撤销已经发生的预付费，磁盘、EIP 等资源仍可能持续计费。
