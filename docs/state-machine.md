...
- **每日汇总**：23:58 推送系统状态、当前节点、切换进度、各节点状态、流量、运行时长与额度预警，兼作心跳（收不到 = 脚本/主机或 TG 链路异常）。
...
- **名称语义**：`accounts` 下的键（如 `A`）是状态机和 `/switch` 使用的稳定账号键；`instance_id` 是 ECS 资源 ID；TG 优先显示可选的 `instance_name`，未配置时自动采用 DescribeInstances 返回的 ECS InstanceName，并附 `[账号键]` 防止重名。
- **命令**：`/check`（手动触发一次与每日任务正文完全一致的汇总，使用最近快照）`/traffic`（入队后在下一次巡检实时调用 CDT API）`/switch`（手动调度，走完整流水线）`/breaker`（二次确认）`/resume` `/last`（最近 10 条事件）。
